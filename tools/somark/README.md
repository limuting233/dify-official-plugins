...
| element_formats | object        | No / 否        | Controls the output format of specific document elements. Supported fields: `image` (`url` / `base64` / `file` / `none`), `formula` (`latex` / `mathml` / `ascii`), `table` (`markdown` / `html` / `image`), `cs` (`image`). When `output_formats` includes `zip`, `element_formats.image` must be omitted or set to `file`; <br> 控制特定文档元素的输出格式。支持字段：`image`（`url` / `base64` / `file` / `none`）、`formula`（`latex` / `mathml` / `ascii`）、`table`（`markdown` / `html` / `image`）, `cs` （`image`）。当 `output_formats` 包含 `zip` 时，`element_formats.image` 必须省略或设置为 `file` |
