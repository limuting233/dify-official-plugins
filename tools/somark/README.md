...
**`somarkdown`** `string` — The parsed document content in SoMarkDown format, which preserves document structure and
element semantics in SoMark's native Markdown representation.

解析后的文档内容（SoMarkDown 格式），以 SoMark 原生的 Markdown 表达方式保留文档结构和元素语义。
