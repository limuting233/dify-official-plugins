...
3.  Configure the SoMark plugin parameters:
    ...
    *   **Deployment Type / 部署方式** (required / 必填): Choose between the official cloud service and a self-hosted SoMark instance.

        选择官方云服务或自建 SoMark 实例。

        | Option / 选项 | When to choose / 何时选择 |
        | :--- | :--- |
        | **SoMark API** | Use the SoMark official service. / 使用 SoMark 官方服务。 |
        | **SoMark Self-host** | You have deployed SoMark on your own infrastructure. / 您已在自有基础设施上部署 SoMark。 |

    *   **Base URL** (required / 必填): The address of the SoMark API endpoint.
        ...
        - For **SoMark Self-host**, fill in your private SoMark server Base URL (e.g. `https://somark.your-domain.com/api/v1`). The URL must start with `http://` or `https://`.

          使用 **SoMark Self-host** 时，填写您私有化部署的 SoMark 服务 Base URL （如 `https://somark.your-domain.com/api/v1`），必须以 `http://` 或 `https://` 开头。

    *   **API Key**:

        - **Required** for **SoMark API** mode. The plugin will validate your key against the SoMark service when you save.

          **SoMark API** 模式下**必填**。保存时插件会请求 SoMark 服务校验 API Key 有效性。
        - **Not required** for **SoMark Self-host** mode (leave it blank if your self-hosted instance does not require it).

          **SoMark Self-host** 模式下**无需填写**（如私有化部署的 SoMark 实例不需要鉴权可留空）。
